    ...
    <header className="sticky top-0 z-40 bg-[#0b1329]/90 backdrop-blur-md border-b border-slate-800/60">
      <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
        ...
        {onSearchChange && (
          <div className="flex-1 max-w-sm hidden sm:block relative">
            ...
            <input
              ...
              className="w-full bg-slate-900/90 border border-slate-800 rounded-xl pr-9 pl-4 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-emerald-500/50 transition-colors"
            />
          </div>
        )}
        ...
        >
          ...
          <span>المحفوظات</span>
          {bookmarkCount > 0 && (
            <span className="bg-emerald-500 text-slate-950 font-bold text-[10px] px-1.5 py-0.5 rounded-full mr-0.5">
              {bookmarkCount}
            </span>
          )}
        </Link>
